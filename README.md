...
AuraSearch 不再只是主题色配置，而是一套可独立运行、复制和转译的完整产品应用参考。它与多主题比较 Demo 分离，拥有自己的 HTML、CSS、JavaScript 和 33 个本地设计资产。

| 能力层级 | 已实现内容 |
|---|---|
| 视觉令牌 | 浅色/暗色、字体栈、字阶、间距、圆角、边界、阴影和数据系列色 |
| 产品外壳 | 264px 圆角侧栏、48px 顶栏、工作区、全局搜索、用户和移动导航 |
| 页面模式 | Dashboard、Prompt Explorer、通用分析、竞品、报告与 Billing & Plans |
| 交互状态 | 路由、筛选、搜索、主从详情、命令面板、时间范围、Toast 和主题持久化 |
| 响应式 | 1240、1024、768 和 520px 四级回退，窄屏使用导航抽屉 |
| 独立性 | 不依赖多主题 Demo、CDN、临时 Figma URL 或第三方前端运行库 |

快速入口：

- [查看 AuraSearch 独立模板源码](demo/aurasearch/)
- [查看 AuraSearch 主题设计规范](references/theme-aurasearch.md)
- [查看完整独立主题交付标准](references/standalone-theme-implementation.md)
...
AuraSearch Demo 是视觉、布局、组件密度和交互的参考实现，不应该作为业务项目的运行时依赖。实际开发时，让 Skill 先读取目标仓库，再把这套设计语言映射到已有技术栈。

### 推荐工作流

1. 使用 Codex 打开目标项目，确认 `$nebula-design-skill` 可以被发现。
2. 明确这是产品应用、营销官网还是完整产品套件；产品应用继续说明是聚焦单页面还是完整系统外壳。
3. 明确写出“使用 AuraSearch 风格”。未指定主题时，Skill 仍会默认使用 Cal.com。
4. 提供用户角色、核心任务、页面范围、主要实体、字段、状态、权限和接口完成度。
5. 要求复用项目已有的框架、路由、状态管理、组件和设计令牌。
6. 让 Codex 直接实现并运行项目现有的 lint、类型检查、测试和多断点验证。

### 完整系统应用提示词

下面的模板可以直接复制，把方括号内容替换为真实业务信息：

```text
$nebula-design-skill

请分析当前仓库的技术栈、路由、组件库和现有设计令牌，
在当前项目中使用 AuraSearch 风格实现一个完整产品应用。
...
设计与实现要求：
- 采用完整系统外壳，包含侧栏、顶栏、工作区和用户信息
- 支持浅色与暗色模式
- 参考 demo/aurasearch 的布局、组件密度和交互方式
- 使用当前项目已有的框架、路由、状态管理和组件库
- 将 AuraSearch 令牌映射到现有设计系统，不创建冲突的第二套令牌
- 不复制 Demo 中的品牌名、示例用户和虚构业务数据
- 连接现有接口；接口尚未完成时使用可替换的类型化 Mock 数据
- 适配桌面端、平板和手机端

完成后运行项目已有的 lint、类型检查和测试，
检查 1440、1024、768 和 390px 布局，
汇报修改文件、已实现交互和仍依赖后端的部分。
```

### 已有系统新增单页面

系统已经有导航和权限框架时，需要明确要求沿用现有外壳：

```text
$nebula-design-skill

分析当前仓库，在现有系统中新增“客户风险详情页”。

使用 AuraSearch 风格，但沿用现有侧栏、顶栏、路由和权限体系，
不要创建第二套系统外壳。
...
使用 AuraSearch 风格创建一个全新的风控运营系统。
这是“产品应用 + 完整系统外壳”，不是营销官网，也不是单页面 Demo。

技术栈使用 React + TypeScript。
请先设计路由、系统外壳、主题令牌和公共组件，
再实现 Dashboard、风险对象、策略管理、告警中心和系统设置。
```

### 在项目中固定默认主题

如果某个仓库长期使用 AuraSearch，可以在该项目的 `AGENTS.md` 中补充：

```md
## 前端设计规范

前端设计和页面实现使用 `nebula-design-skill`。

除非需求另有指定：
- 产品应用默认采用 AuraSearch 主题。
- 已有系统优先复用现有导航、路由、组件库和设计令牌。
- `demo/aurasearch` 只作为视觉与交互参考，不作为运行时依赖。
- 不复制 Demo 的品牌标识、示例用户和业务数据。
- 新页面覆盖响应式、深浅色、加载、空态、失败和权限状态。
```

### 集成边界

- **已有项目**：优先做令牌映射和组件适配，不整体复制 Demo，也不替换业务框架。
- **全新项目**：可以把独立模板作为页面结构参考，但仍应按选定框架拆分路由、组件和状态。
- **品牌与资产**：生产项目应替换 AuraSearch Logo、示例头像和演示文案；继续使用导出资产前确认原 UI Kit 授权。
- **后端接口**：Demo 数据只用于验证界面，生产状态、权限和错误处理必须以真实接口契约为准。
...
| Demo | 源码入口 | 本地预览 | 用途 |
|---|---|---|---|
| 多主题产品应用 | [demo/index.html](demo/index.html) | [打开产品应用](http://localhost:8765/demo/index.html) | 比较同一风控关系工作台在 8 套主题下的效果 |
| 多主题营销官网 | [demo/marketing.html](demo/marketing.html) | [打开营销官网](http://localhost:8765/demo/marketing.html) | 比较同一营销叙事在 8 套官网视觉语言下的效果 |
| AuraSearch 独立产品模板 | [demo/aurasearch/index.html](demo/aurasearch/index.html) | [打开 AuraSearch](http://localhost:8765/demo/aurasearch/) | 查看完整系统外壳、多页面、深浅色和响应式交互 |
...
启动服务后，点击上表“本地预览”链接即可交互查看。GitHub 页面中的“源码入口”用于查看实现文件，本地预览链接需要先运行上述服务。
