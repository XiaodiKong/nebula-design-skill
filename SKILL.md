...
description: Design, implement, review, or optimize desktop-first Chinese enterprise admin products and HTML-based technical diagrams with selectable Clarity, Cal.com, Notion, or Linear-inspired visual themes. Use for dashboards, data platforms, CRM/customer profiles, risk-control systems, workflow/configuration tools, permission management, AI-assisted workbenches, tables, forms, detail pages, drawers, system architecture diagrams, hierarchy trees, flowcharts, swimlanes, data lineage DAGs, network topologies, sequence diagrams, relationship graphs, and other complex back-office interfaces; also use when turning business requirements into information architecture, interaction specifications, UI copy, reusable components, theme tokens, SVG/Canvas diagram workbenches, or production frontend code.
---

# Backend Product Design

把模糊业务需求转化为清晰、可操作、可实现的中文企业级中后台。支持 Clarity、Cal.com、Notion 和 Linear 四套主题；Clarity 是默认主题。无论选择哪种视觉语言，都优先保证任务效率、信息层级、状态语义和可访问性。
...
- 用户选择 Clarity 或未指定主题：使用 [references/visual-system.md](references/visual-system.md) 中的默认主题规则。
...
- 新项目需要基础视觉变量时，从 [references/theme-selection.md](references/theme-selection.md) 指向的四套 `assets/*-tokens.css` 中选择一套；已有设计令牌时映射 `--admin-*` 语义，不要并行维护第二套冲突令牌。
...
用户明确指定主题时直接使用。用户要求提供风格选择时，使用同一业务内容制作四套可比较预览或主题切换器；不要用不同信息架构制造虚假的风格差异。

用户未指定时：

- 风控、数据、权限和高密度运营后台默认 Clarity。
- 轻量 SaaS、预约和表单流程可建议 Cal.com。
- 知识、内容、协作和项目工作区可建议 Notion。
- 暗色工程、研发、自动化和运维工具可建议 Linear。

自定义字体只有在项目已有合法资产时使用。Cal.com、Notion 和 Linear 是基于公开页面分析的非官方适配，不能声称使用官方设计系统。
